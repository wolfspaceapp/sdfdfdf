(function () {
    ...
    // Solo filtra si el ID es un número válido (> 0)
    const uniqueData = (arr) => {
        const seen = new Set();
        return arr.filter(item => {
            if (!item) return false;
            // Si el ID no es válido (0, null, undefined), siempre incluir la entrada
    ...
    function navigateTo(view, back = false) {
        if (view === 'featured-all') {
            ...
            // detail is deprecated and its render logic removed, fallback to home
            view = 'home';
        }
        ...
        // Failsafe: Si la vista no existe (ej. se eliminó la vista detail), volver al home para evitar pantalla negra
        if (!next) {
            console.warn('[navigateTo] Vista no encontrada:', view, '- Redirigiendo a home');
            if (view !== 'home') {
        ...
        if (state.view === view && next.classList.contains('active')) return true;
